...
# Custom CSS for sidebar + tables
st.markdown(
    """
    <style>
      section[data-testid="stSidebar"] { width: 320px !important; }
      section[data-testid="stSidebar"] > div { padding-right: 8px; }
      .stSelectbox [data-baseweb="select"] { min-height: 38px; }
...
if "DATABASE_URL" not in st.secrets:
    st.error("❌ Missing secret `DATABASE_URL`")
    st.stop()
...
def sidebar_controls():
    ...
    st.sidebar.markdown(" ")
    do_lookup = st.sidebar.button("Lookup", type="primary", disabled=not (equipment and component and task_name))
    clear = st.sidebar.button("Clear selections")

    if clear:
        st.session_state.last_lookup = None
        st.rerun()

    return equipment, component, task_name, do_lookup

equipment, component, task_name, do_lookup = sidebar_controls()
...
if do_lookup:
    ...
    if df.empty:
        ...
        cost_col = "labour_rate" if "labour_rate" in df.columns else None
        if cost_col:
